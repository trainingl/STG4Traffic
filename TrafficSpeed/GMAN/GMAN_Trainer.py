...
class Trainer(object):
    def __init__(self, args, data_loader, scaler, model, loss, optimizer, lr_scheduler):
    ...
    def train_epoch(self):
        ...
        for _, (x, y, TE) in enumerate(self.train_loader.get_iterator()):
            trainx = torch.Tensor(x).to(self.args.device)
            trainy = torch.Tensor(y).to(self.args.device)
            TE = torch.Tensor(TE).to(self.args.device)
            self.optimizer.zero_grad()
            output = self.model(trainx, self.args.SE, TE)
            # 预测的是归一化的结果, 所以需要反归一化
    ...
    def val_epoch(self):
        ...
        self.model.eval()
        with torch.no_grad():
            for _, (x, y, TE) in enumerate(self.val_loader.get_iterator()):
                validx = torch.Tensor(x).to(self.args.device)
                validy = torch.Tensor(y).to(self.args.device)
                TE = torch.Tensor(TE).to(self.args.device)
                output = self.model(validx, self.args.SE, TE)  # (B, T, N)
                # 预测的是归一化的结果, 所以需要反归一化
                predict = self.scaler.inverse_transform(output)
    ...
    def test(self, args, model, data_loader, scaler, logger, save_path=None):
        ...
        realy = []   # (B, T, N)
        with torch.no_grad():
            for _, (x, y, TE) in enumerate(data_loader['test_loader'].get_iterator()):
                testx = torch.Tensor(x).to(args.device)
                testy = torch.Tensor(y).to(args.device)
                TE = torch.Tensor(TE).to(args.device)
                preds = self.model(testx, self.args.SE, TE)   # (B, T, N, 1)
                outputs.append(preds)
                realy.append(testy)
        
        yhat = torch.cat(outputs, dim=0)   # concat at batch_size
        ...
        for i in range(args.horizon):
            # 预测的是归一化的结果, 所以需要反归一化
            pred = scaler.inverse_transform(yhat[:, i, :,])  # (B, T, N)
            real = realy[:, i, :]  # (B, T, N)
