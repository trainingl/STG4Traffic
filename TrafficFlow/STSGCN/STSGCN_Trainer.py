...
class Trainer(object):
    def __init__(self, args, data, train_loader, val_loader, test_loader, scaler, model, loss, optimizer, lr_scheduler=None):
    ...
    def train_epoch(self):
        ...
        for _, (x, y) in enumerate(self.train_loader.get_iterator()):
            ...
            loss = self.loss(predict, trainy[:, :, :, 0:1])
            loss.backward()
    ...
    def val_epoch(self):
        ...
        with torch.no_grad():
            for _, (x, y) in enumerate(self.val_loader.get_iterator()):
                valx = torch.Tensor(x).to(self.args.device)
                valy = torch.Tensor(y).to(self.args.device)
                output = self.model(valx)
                predict = self.scaler.inverse_transform(output)
                mae = masked_mae(predict, valy[:, :, :, 0:1], 0.0).item()
                rmse = masked_rmse(predict, valy[:, :, :, 0:1], 0.0).item()
                mape = masked_mape(predict, valy[:, :, :, 0:1], 0.0).item()
    ...
    def train(self):
        ...
        self.test(self.model, self.args, self.dataloader, self.scaler, self.logger)
