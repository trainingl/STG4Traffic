...
from model.STGNCDE.controldiffeq import interpolate, cdeint_module
...
class NeuralGCDE(nn.Module):
    ...
    def forward(self, times, coeffs):
        ...
        spline = interpolate.NaturalCubicSpline(times, coeffs)
        if self.init_type == 'fc':
        ...
        z_t = cdeint_module.cdeint_gde_dev(dX_dt=spline.derivative, #dh_dt
                                   h0=h0,
                                   z0=z0,
                                   func_f=self.func_f,
                                   func_g=self.func_g,
