...
class Trainer(object):
    def __init__(self, args, train_loader, val_loader, test_loader, scaler, times, model, loss, optimizer, lr_scheduler=None):
        ...
        self.times = times.to(args.device, dtype=torch.float)
        # 日志与模型的保存路径
    ...
    def train_epoch(self):
        ...
        for _, batch in enumerate(self.train_loader):
            batch = tuple(b.to(self.args.device, dtype=torch.float) for b in batch)
            *train_coeffs, target = batch
            # data = data[..., :self.args.input_dim]
            label = target[..., :self.args.output_dim]
            # data and target shape: B, T, N, D; output shape: B, T, N, D
            self.optimizer.zero_grad()
            output = self.model(self.times, train_coeffs)  # directly predict the true value
    ...
    def train(self):
        ...
        self.test(self.model, self.args, self.test_loader, self.scaler, self.logger, self.times)


    @staticmethod
    def test(model, args, data_loader, scaler, logger, times, save_path=None):
        if save_path != None:
